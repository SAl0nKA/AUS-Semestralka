...
namespace ds::amt {
    ...
    public:
        ...
        BlockType* accessLastSon(const BlockType& node) const;
    ...
    //niki was here
    template<typename DataType>
    auto MultiWayExplicitHierarchy<DataType>::accessLastSon(const BlockType& node) const -> BlockType* {
        MemoryBlock<BlockType*>* sonBlock = node.sons_->accessLast();
        return sonBlock != nullptr ? sonBlock->data_ : nullptr;
    }
    ...
    //niki was here
    template<typename DataType>
    auto MultiWayExplicitHierarchy<DataType>::emplaceLastSon(BlockType& parent) -> BlockType& {
        BlockType* newSon = AbstractMemoryStructure<BlockType>::memoryManager_->allocateMemory();
        parent.sons_->insertLast().data_ = newSon;
        newSon->parent_ = &parent;
        return *newSon;
    }
